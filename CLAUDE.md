...
|---|---|---|
| ESLint | ✅ **PR 作成前に実行** (`pnpm lint`) | ✅ `pnpm lint` |
| ビルド | **禁止** (deny) | ✅ `pnpm build` |
| 型チェック (tsc) | ✅ コミット前に実行 | ✅ `pnpm type-check` |
| テスト (vitest) | ✅ ロジック修正時に実行 | ✅ `pnpm test` |
| E2E テスト (Playwright) | ✅ **push 前に `pnpm test:e2e:local` を実行** | ✅ `e2e` ジョブ |
| Stylelint | ✅ スタイル修正時に手動可 | — |

**Claude Code はコミット前に `pnpm type-check` を実行する。PR 作成前に `pnpm lint` を実行してエラーを修正してから PR を作成する。build は実行しない。**

prettier・import/order エラーは `pnpm lint --fix` で自動修正できる場合がある。エラーが残る場合は手動で修正してから PR を作成すること。

**E2E テストは push 前にローカルで `pnpm test:e2e:local` を実行して通過を確認すること。**
このコマンドは Firebase エミュレーターの起動・停止を自動管理するため、別途 `pnpm mock` は不要。
